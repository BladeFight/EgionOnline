...
public class AtavismScriptEvent : MonoBehaviour
{
	...
	public void _HandleComm (BaseWorldMessage message)
	{
		...
		Debug.Log("Got comm message with channel: " + commMessage.ChannelId);
		if (commMessage.ChannelId == 0) {
			...
			EventSystem.DispatchEvent("CHAT_MSG_SERVER", args);
			...
			args[2] = "";
			EventSystem.DispatchEvent("CHAT_MSG_SAY", args);
			...
			args[1] = "System";
			args[2] = "";
			EventSystem.DispatchEvent("CHAT_MSG_SYSTEM", args);
		}
	}
	...
	public void _HandleObjectProperty(BaseWorldMessage message) {
		...
		if (propMessage.Properties.Count <= 0)
        	return;
    	ObjectNode target = ClientAPI.GetTargetObject();
    	Player player = ClientAPI.GetPlayerObject();
    	//pet = None;
	...
	public void HandleCombatText2(Dictionary<string, object> props) {
		...
		ObjectNode target = ClientAPI.WorldManager.GetObjectNode(targetOID);
