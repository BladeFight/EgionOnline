...
public class Inventory : MonoBehaviour {
	...
	void Start() {
		...
		displayPrefabs = Resources.LoadAll("Content/Items");
		foreach (Object displayPrefab in displayPrefabs) {
			GameObject go = (GameObject) displayPrefab;
		...
		displayPrefabs = Resources.LoadAll("Content/Currencies");
		foreach (Object displayPrefab in displayPrefabs) {
			GameObject go = (GameObject) displayPrefab;
	...
	public void HandleBagInventoryUpdate(Dictionary<string, object> props) {
    	...
    	for (int i = 0; i < numBags; i++) {
			...
			Debug.Log("Got bag with name: " + bag.name);
    	...
    	for (int i = 0; i < numItems; i++) {
        	...
        	AtavismInventoryItem invInfo = LoadItemPrefabData(baseName);
         	UnityEngine.Debug.Log("Got item: " + invInfo.BaseName);
    	...
    	EventSystem.DispatchEvent("INVENTORY_UPDATE", args);
	}
	...
	public void HandleEquippedInventoryUpdate(Dictionary<string, object> props) {
		...
		for (int i = 0; i < numSlots; i++) {
			...
			Debug.Log("Added equipped item: " + invInfo.name + " to slot: " + i);
		}
		string[] args = new string[1];
    	EventSystem.DispatchEvent("EQUIPPED_UPDATE", args);
	}
	...
	public void HandleCurrencies(Dictionary<string, object> props) {
		...
		string[] args = new string[1];
		EventSystem.DispatchEvent("CURRENCY_UPDATE", args);
	}
	
	public void HandleLootList(Dictionary<string, object> props) {
		loot.Clear();
		int numItems = (int)props["numItems"];
		Debug.Log("Got Loot list with num items: " + numItems);
		...
		for (int i = 0; i < numItems; i++) {
			...
			loot.Add(i, invInfo);
			Debug.Log("Added loot item: " + invInfo.name + " to slot: " + i);
		}
		string[] args = new string[1];
    	EventSystem.DispatchEvent("LOOT_UPDATE", args);
	}
