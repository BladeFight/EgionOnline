...
public class BagBar : AtavismWindowTemplate {
			...
			if (i == 0) {
				...
				if (GUI.Button(bagButtonRect, bagData.icon)) {
					...
					if (Event.current.button == 0) {
						...
						GetComponent<Cursor>().PickupBag(bagData);
					}
				}
			} else {
				if (GUI.Button(bagButtonRect, "", GUIStyle.none)) {
					if (GetComponent<Cursor>().CursorHasItem()) {
						GetComponent<Cursor>().PlaceCursorItemAsBag(i);
					} else if (GetComponent<Cursor>().CursorHasBag()) {
						GetComponent<Cursor>().PlaceCursorBag(i);
					}
				}
			}
			if (bagOpen[i]) {
		...
		for (int i = 0; i < bagData.numSlots; i++) {
			...
			if (bagData.items.ContainsKey(i)) {
				if (GUI.Button(buttonRect, bagData.items[i].icon)) {
					if (Event.current.button == 1) {
						if (bagData.items[i].Count > 0)
							GetComponent<Cursor>().PickupBagItem(slot, i, bagData.items[i]);
					} else if (Event.current.button == 0) {
						// Check for interface overrides
						if (gameObject.GetComponent<Cursor>().InterfaceStateOverriden()) {
							gameObject.GetComponent<Cursor>().HandleItemUseOverride(bagData.items[i]);
						} else {
				...
				if (GUI.Button(buttonRect, ""/*, GUIStyle.none*/)) {
					GetComponent<Cursor>().PickupBagItem(slot, i, null);
				}
			}
		}
	...
	public void OpenBag(int slot) {
		...
		UiSystem.AddFrame("Bag" + slot, bagRect);
		bagOpen[slot] = true;
	...
	void CloseBag(int slot) {
		...
		UiSystem.RemoveFrame("Bag" + slot, new Rect(0, 0, 0, 0));
		bagOpen[slot] = false;
	}
