...
public class MerchantUI : AtavismWindowTemplate {
	...
	void Start () {
		...
		// Register for 
		EventSystem.RegisterEvent("MERCHANT_UPDATE", this);
		EventSystem.RegisterEvent("SELL_ITEM", this);
	}
	
	void OnDestroy () {
		EventSystem.UnregisterEvent("MERCHANT_UPDATE", this);
		EventSystem.UnregisterEvent("SELL_ITEM", this);
	}
	...
	void OnGUI() {
		...
		for (int i = 0; i < columnsPerPage; i++) {
			...
			for (int j = 0; j < rowsPerPage; j++) {
				...
				if (buttonRect.Contains(mousePosition)) {
					mouseOverItem = true;
					gameObject.GetComponent<Cursor>().MerchantItemEntered();
		...
		if (!mouseOverItem) {	
			gameObject.GetComponent<Cursor>().MerchantItemLeft();
		}
		...
		if (mState == MerchantState.Purchase) {
			...
			GUI.Label(new Rect(confirmBoxRect.x + 5, confirmBoxRect.y + 5, confirmBoxRect.width-10, 60), 
			          "Do you want to sell " + itemBeingSold.name + " for " + itemBeingSold.Cost + " " + currencyName);
			if (GUI.Button(new Rect(confirmBoxRect.x + 5, confirmBoxRect.yMax - 25, 35, 20), "Yes")) {
	...
	void ShowPurchaseWindow(MerchantItem mItem) {
		...
		selectedItem = mItem;
		UiSystem.AddFrame("MerchantDialog", uiRect);
	}
	...
	void HideMerchantDialog() {
		...
		UiSystem.RemoveFrame("MerchantDialog", new Rect(0, 0, 0, 0));
	}
	...
	public void ToggleOpen() {
		...
		if (open) {
			...
			gameObject.GetComponent<Cursor>().ChangeMerchantState(true);
		} else {
			UiSystem.RemoveFrame(frameName, new Rect(0, 0, 0, 0));
			UiSystem.RemoveFrame("MerchantDialog", new Rect(0, 0, 0, 0));
			gameObject.GetComponent<Cursor>().ChangeMerchantState(false);
		}
	}
}
