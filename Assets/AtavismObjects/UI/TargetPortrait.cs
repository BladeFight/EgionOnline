...
public class TargetPortrait : AtavismWindowTemplate {
	...
	void Start () {
		SetupRect();
		// Register for 
		EventSystem.RegisterEvent("PLAYER_TARGET_CHANGED", this);
		EventSystem.RegisterEvent("PROPERTY_health", this);
		EventSystem.RegisterEvent("PROPERTY_health-max", this);
	...
	void OnDestroy () {
		EventSystem.UnregisterEvent("PLAYER_TARGET_CHANGED", this);
		EventSystem.UnregisterEvent("PROPERTY_" + health_prop, this);
		EventSystem.UnregisterEvent("PROPERTY_" + health_max_prop, this);
	}
