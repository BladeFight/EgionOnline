...
/*
 * Developers will be able to set what players characters start off with in regards to stats, items and skills. The way it works is the developer sets up a template for a race/aspect combination. So for a Human Warrior they have a certain set of stats, items and skills given to them upon creation. Then for an Orc warrior there will be another set of stats, items and skills etc.

The UI for this will be quite complicated as it does not have a set number of entries. A template needs a value set for every stat, then can have as many skills and items set to it.

The data is split up into multiple tables:

Core table - character_create_template:

id - Integer - auto generated by the database
race - String - the race for this character template
aspect - String - the aspect (class) for this character template
instanceName - String - the name of the scene the character will spawn in first time the player logs in
pos_x - Float - value on the x axis where they will spawn (not used yet)
pos_y - Float - value on the y axis where they will spawn (not used yet)
pos_z - Float - value on the z axis where they will spawn (not used yet)
orientation - Float - yaw value representing where they will face when spawned (not used yet)
health - Integer - the starting health of the character
mana - Integer - the starting mana of the character
Stats table - character_create_stats

id - Integer - auto generated by the database
character_create_id - Integer - links to the character_create_template
stat - String - the name of the stat
value - Integer - how much of the stat the character starts with
Skills table - character_create_skills

id - Integer - auto generated by the database
character_create_id - Integer - links to the character_create_template
skill - Integer - the ID of the skill to be given to the character (links to the skills database)
Items table - character_create_items

id - Integer - auto generated by the database
character_create_id - Integer - links to the character_create_template
item_id - Integer - the ID of the item to be given to the character (links to the items database)
value - Integer - how many of the item the character gets
equipped - Boolean - is the item equipped on the character
